...
# the function action_list_to_action_events() will transform
# ActionRecordUnity into ActionEvents
...
class ActionEvent(NamedTuple):
    ...
    def __repr__(self):
        fields_to_show = [
            x for x in self._fields if x not in [
                'xy_wrt_bounding_box',
                'xy_wrt_world',
                'frames']]
        with np.printoptions(precision=3, suppress=True):
            return 'ActionEvent(' + ', '.join(
                [f'{x}={getattr(self, x)}' for x in fields_to_show]) + ')'
...
def new_action_record(action_dict: dict) -> ActionRecordUnity:
    ...
    action_dict.update(
        dict(
            ...
            collected_value=action_dict['collected_value'] if action_dict['collected_value'] > -
            9999 else np.nan))
    action = ActionRecordUnity(**action_dict)
    return action


def action_list_to_action_events(
        actions_from_unity: list[dict] | list[ActionRecordUnity],
    ...
    if all([isinstance(x, dict) for x in actions_from_unity]):
        ...
        actions = [new_action_record(cast(dict, action_dict))
                   for action_dict in actions_from_unity]
    ...
    action_events = []
    mouse_ups = np.where(
        [action.action_type.name == ActionType.MOUSE_UP.name for action in actions])[0]

    # one group of actions is one MOUSE_DOWN_START, several MOUSE_DOWN, and
    # one MOUSE_UP
    mouse_ups = [-1] + mouse_ups.tolist()
    # list of lists, each list is a sequence of action events ending with a
    # mouse up event
    group_actions_by_mouse_up = [
        actions[mouse_ups[x - 1] + 1:mouse_ups[x] + 1] for x in range(1, len(mouse_ups))]

    for grouped_actions in group_actions_by_mouse_up:
        # each grouped action should be MOUSE_DOWN_START, a number of
        # MOUSE_DOWN, and then a MOUSE_UP
        if len(grouped_actions) <= 1:
            ...
            if mouse_down_start.action_type == ActionType.MOUSE_DOWN_START and mouse_up.action_type == ActionType.MOUSE_UP:
                match [mouse_up.raycasted_objects_during_collect, mouse_up.clicked_object]:
                    case [[], '']:
                        # there was no raycasting happening -> the spotlight
                        # was not clicked but sth else (e.g., empty space)
                        targeted_object = 'empty'
                    case [[], clicked_object]:
                        # there was no raycasting happening -> the spotlight
                        # was not clicked but sth else (e.g., occluder)
                        targeted_object = clicked_object
                    case [raycasted_objects, clicked_object]:
                        # there was raycasting and one of the objects during
                        # raycasting is a target or a distractor
                        targets_or_distractors = [
                            x for x in raycasted_objects if target_str in x or distractor_str in x]
                        if len(targets_or_distractors) > 1:
                            print(
                                'there should actually only be one target or one distractor in the raycasting list (during spotlight collect())')
                    ...
                    case _:
                        raise Exception(
                            f"something does not match. clicked_object={mouse_up.clicked_object} || raycasted_objects_during_collect={raycasted_objects}")

            if any([occluder_str in obj_name for obj_name in mouse_up.raycasted_objects_during_collect]
                   ) or occluder_str in mouse_up.clicked_object:
            ...
            action_event = ActionEvent(
                ...
                clicked_on_occluder=clicked_on_occluder,
                start=start,
                end=end,
                duration=duration,
...
def process_responses_from_actionevent_list(
    ...
    responses_df = pd.DataFrame(
        columns=[
            ...
            'response_at',
            'rt',
            'targeted_object_name',
            'x',
            'y'])

    responses_to_filtered_objects = [
        r for r in action_events if r.targeted_valued_object]

    if sort_responses_by_timestamp:
        timestamps = [r2fo.start for r2fo in responses_to_filtered_objects]
        responses_to_filtered_objects = [x for _, x in sorted(
            zip(timestamps, responses_to_filtered_objects))]
...
def classic_mot_process_trial_data(
        trial_list: list[dict], single_trial_metric_funs: dict[str, Callable]) -> list:

    trial_blueprint_information_list = []

    single_trial_metrics = {metric_name: []
                            for metric_name in single_trial_metric_funs.keys()}
...
if __name__ == '__main__':
    ...
    single_trial_metrics = classic_mot_process_trial_data(
        trial_list, dict(collected_coins=collected_coins, accuracy=accuracy))
