...
class ActionEvent(NamedTuple):
    ...
    def __repr__(self):
        fields_to_show = [
            ...
            if x not in ["xy_wrt_bounding_box", "xy_wrt_world", "frames"]
        ]
        with np.printoptions(precision=3, suppress=True):
            return (
                "ActionEvent("
                + ", ".join([f"{x}={getattr(self, x)}" for x in fields_to_show])
                + ")"
            )
...
def new_action_record(action_dict: dict) -> ActionRecordUnity:
    ...
    action_dict.update(
        dict(
            ...
            collected_value=action_dict["collected_value"]
            if action_dict["collected_value"] > -9999
            else np.nan,
        )
    )
...
def action_list_to_action_events(
    ...
    if all([isinstance(x, dict) for x in actions_from_unity]):
        ...
        actions = [
            new_action_record(cast(dict, action_dict))
    ...
    action_events = []
    mouse_ups = np.where(
        [action.action_type.name == ActionType.MOUSE_UP.name for action in actions]
    )[0]
    ...
    group_actions_by_mouse_up = [
        actions[mouse_ups[x - 1] + 1 : mouse_ups[x] + 1]
        for x in range(1, len(mouse_ups))
    ]  # list of lists, each list is a sequence of action events ending with a mouse up event
    ...
    for grouped_actions in group_actions_by_mouse_up:
        ...
        if len(grouped_actions) <= 1:
            ...
            if (
                mouse_down_start.action_type == ActionType.MOUSE_DOWN_START
                and mouse_up.action_type == ActionType.MOUSE_UP
            ):
                match [
                    mouse_up.raycasted_objects_during_collect,
                    mouse_up.clicked_object,
                ]:
                    case [[], ""]:
                    ...
                    case [raycasted_objects, clicked_object]:
                        ...
                        targets_or_distractors = [
                            x
                            for x in raycasted_objects
                            if target_str in x or distractor_str in x
                        ]
            ...
            if (
                any(
                    [
                        occluder_str in obj_name
                        for obj_name in mouse_up.raycasted_objects_during_collect
                    ]
                )
...
def process_responses_from_actionevent_list(
    ...
    rt_baseline: int = 0,
    sort_responses_by_timestamp: bool = True,
) -> pd.DataFrame:
    ...
    if sort_responses_by_timestamp:
        ...
        responses_to_filtered_objects = [
            x for _, x in sorted(zip(timestamps, responses_to_filtered_objects))
        ]
...
def classic_mot_process_trial_data(
    trial_list: list[dict], single_trial_metric_funs: dict[str, Callable]
) -> list:
    trial_blueprint_information_list = []

    single_trial_metrics = {
        metric_name: [] for metric_name in single_trial_metric_funs.keys()
    }
