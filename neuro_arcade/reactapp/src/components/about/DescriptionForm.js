...
export function DescriptionForm({description}) {
    ...
    function editDescription(e) {
        e.preventDefault();
        getAboutData().then((data) => setNewDescription(data.description));
    ...
    return (
        <>
            ...
            <motion.button id={styles["edit"]} onClick={editDescription} whileHover={{scale: 1.1}} whileTap={{scale: 0.9}}>
                {displayTextEdit ? (
                    <>
                        cancel
                        <div>
            ...
            {displayTextEdit ? (
                <motion.button onClick={onSave} whileHover={{scale: 1.1}} whileTap={{scale: 0.9}} id={styles["save"]}>
                    save
                    <div>
                        <FaSave />
                    </div>
